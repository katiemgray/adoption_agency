...
class AddPetForm(FlaskForm):
    ...
    name = StringField("Pet Name", validators=[InputRequired(), Length(min=0, max=50)])
    species = StringField("Species Name", validators=[InputRequired(), AnyOf(values=['cat', 'dog', 'porcupine'])])
    photo_url = StringField("Photo Url", validators=[Optional(), URL()])
    age = IntegerField("Pet Age", validators=[InputRequired(), NumberRange(min=0, max=30)])
    notes = TextAreaField("Pet Notes", validators=[Optional(), Length(min=0, max=300)])
